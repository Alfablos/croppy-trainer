...
1. ~~retrain with 1024x768: small errors in 512x384 would become huge errors in 6016x4512. 1 pixel in 512x384 is 0.195% but a 19% error is 11.4 pixels! Nope, wrong approach, 512x512 is enough.~~
...
8. ~~**shrink the corners** before storing them in the dataabase: if someone asked me to crop an image of a document, I WOULD NOT crop it EXACTLY at the borders of the document, I'd sacrifice a little bit of page to 1. exclude page/border imperfections that would reveal the background and 2. because this way, even if the model makes a mistake, there's a higher chance that it will not output coordinates of corners outside the borders of the document~~ => Wrong approach, this can be done post.
...
12. ~~handle several ratios (4:3, 16:9). Done by adding padding after resizing to a standard size (512x512)~~
13. Tensorboard: display debug images
14. ~~The number of parameters for training has become too high. Change approach and use a YAML config file (no more cli). In the code use a config factory: `class ConfigSource(Enum)`:~~ => Wrong approach, use config.py instead.
...
15. ~~Make crawl multithreaded~~
16. ~~Store original coordinates, only compute recess at runtime! => the user already has the option to define --recess (default=0)~~
17. use `config.py` as a configuration source. Generate it with nix for different environments/video cards, which dictate:
    1. What transforms go on the CPU and what go on the GPU
    2. batch-size and other parameters
19. Centralize hyperparameters in `config.py`
20. Improve parameters initialization + resume
