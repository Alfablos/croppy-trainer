...
class Architecture(Enum):
    ...
    def _process_image(path, h, w, color: bool = True, resize: bool = True, interpolation=cv2.INTER_AREA):    
        imdata = cv2.imread(path, cv2.IMREAD_COLOR_RGB if color else cv2.IMREAD_GRAYSCALE)
        if not resize:
            return imdata
        if imdata is None: raise RuntimeError(f"Could not read image at {path}.")
        img_resized = resize_img(imdata, h, w)
        if img_resized is None: raise RuntimeError(f"Could not resize image at {path}.")
    ...
    def _transform_resnet(row, h: int, w: int, precision: Precision):
        ...
        """
        img_resized = Architecture._process_image(row['image_path'], h, w, resize=True, color=True)
    ...
    def get_csv_header(self) -> list[str]:
        if self == Architecture.RESNET:
            return ["index", "path"] + [f"{axis}{i}" for i in range(1, 5) for axis in ('x', 'y')] # [f"c{k}" for k in range(8)]
        elif self == Architecture.UNET:
            return ["index", "path"]
        else:
            assert_never(self)
