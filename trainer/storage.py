...
class LMDBStore(DataStore):
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback):
        if self.transaction:
            if exc_type:
                ...
                try:
                    if self._write:
                        self.transaction.put('__len__'.encode('utf-8'), self.len.to_bytes(64, 'big'))
    ...
    def compact(self, dst_path: str):
        ...
        self.transaction.put('h'.encode('utf-8'), str(self.metadata['h']).encode('utf-8'))
        self.transaction.put('w'.encode('utf-8'), str(self.metadata['w']).encode('utf-8'))
        self.transaction.commit()
    ...
    def __enter__(self):
        if self._write:
            ...
            if not self.path_exists:
                raise FileNotFoundError(f'Cannot create arrow store from file at {self._path}: file not found.')
