...
from common import Device, Purpose
...
def precompute(
    ...
    db_path = str(output_dir) + f"/data_{architecture.value}_{str(precision)}.lmdb"
    if os.path.exists(db_path):
        raise FileExistsError(db_path)
    index_path = str(output_dir) + f"/index_{architecture.value}_{str(precision)}.csv"
